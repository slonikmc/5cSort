...
long long getDigitSortNComp(int *a, const size_t n) {
    long long nComps = 0;
    int step = 8;
    int mask = 1 << step;
    int *buffer = (int *) malloc(sizeof(int) * n);
    int *values = (int *) malloc(sizeof(int) * mask);
    for (int byte = 0; byte < sizeof(int) && ++nComps; byte++) {
        for (int i = 0; i < mask && ++nComps; i++)
            values[i] = 0;

        for (int i = 0; i < n && ++nComps; i++)
            values[a[i] >> (byte * step) & (mask - 1)]++;

        for (int i = 1; i < mask && ++nComps; i++)
            values[i] += values[i - 1];

        for (int i = (int)n - 1; i >= 0 && ++nComps; i--)
            buffer[--values[a[i] >> (byte * step) & (mask - 1)]] = a[i];
